...
class RateLimitService {
     ...
     */
    static async checkAndIncrement(senderId, hourlyLimit) {
        ...
        const result = await redis_1.redisConnection.eval(LUA_CHECK_AND_INCREMENT, 1, // number of KEYS
        key, // KEYS[1]
        hourlyLimit, // ARGV[1]
        TTL_SECONDS // ARGV[2]
        );
    ...
    static async getCurrentCount(senderId) {
        const key = RateLimitService.getKey(senderId);
        const value = await redis_1.redisConnection.get(key);
     ...
     */
    static async getTTL(senderId) {
        const key = RateLimitService.getKey(senderId);
        return redis_1.redisConnection.ttl(key);
    }
}
